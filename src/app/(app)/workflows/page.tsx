...
import { Plus, Search, Folder, Workflow, Play, Pause, MoreHorizontal, FolderOpen, Megaphone, Headphones, FileText } from "lucide-react";
...
export default function WorkflowsPage() {
  ...
  const filtered = activeFolder === "All" ? workflows : workflows.filter((w) => w.folder === activeFolder);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filtered.map((w) => (
              <Link key={w.id} href="/workflows/builder">
                <Card className="p-5 hover:shadow-md hover:border-blue-200 transition-all h-full">
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-4 pt-4 border-t border-slate-100">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-medium text-slate-700">{w.lastUpdated}</p>
                    </div>
                  </div>
                </Card>
              </Link>
