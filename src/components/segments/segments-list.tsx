...
import { Plus, Send, Workflow, Download, RefreshCw, UserPlus, Sparkles, Tags, MoreHorizontal, Search, Trash2 } from "lucide-react";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Card } from "@/components/ui/card";
import { PageHeader } from "@/components/ui/page-header";
import { deleteSegment, type SegmentRow } from "@/lib/queries/segments";
...
export function SegmentsList({ segments }: { segments: (SegmentRow & { contacts: number })[] }) {
  ...
  async function handleDelete(id: string) {
    ...
    start(async () => { await deleteSegment(id); });
  }
